    ...
    def _make_layer(self, block, planes, blocks, stride=1, dilate=False):
        ...
        if stride != 1 or self.inplanes != planes * block.expansion:
            if stride == 1:
                ...
                self.init_layer(downsample[0])
                self.init_bn(downsample[1])
            elif stride == 2:
                downsample = nn.Sequential(
                ...
                self.init_layer(downsample[1])
                self.init_bn(downsample[2])
...
class ResNet38(nn.Module):
    def __init__(self, sample_rate=32000, pretrained=None):
        ...
        if pretrained:
            ...
            self.conv_block1.apply(self.init_weight)
            self.conv_block_after1.apply(self.init_weight)
...
class Ast(nn.Module):
    ...
    def __init__(self,
        ...
        if init_norm:
            self.bn0 = nn.BatchNorm2d(input_fdim)
            self.init_bn(self.bn0)
        ...
        if verbose == True:
            ...
            print(f"ImageNet pretraining: {imagenet_pretrain}, "
                  f"AudioSet pretraining: {audioset_pretrain}.")
        ...
        # now load a model that is pretrained on both ImageNet and AudioSet
            ...
            audio_model = Ast(fstride=fstride, tstride=tstride,
            ...
            self.v.patch_embed.num_patches = num_patches
            if verbose:
                print(f'frequncey stride={fstride}, time stride={tstride}')
                print(f'number of patches={num_patches}')
